...
function MainApp({ token }) {
  ...
  const handleSelectSuggestion = async (suggestion) => {
    ...
    try {
      ...
      // Mocking TM2 and Biomed codes for display based on screenshot
      const candidates = response.data.candidates;
      if (candidates && candidates.length > 0) {
          setIcdResults({
              tm: candidates[0], // Assuming first is TM
              biomed: candidates[1] || candidates[0], // Assuming second is Biomed, or fallback to first
          });
      }
    } catch (error) {
  ...
  const handleGenerateFhir = async () => {
      ...
      const payload = {
        patient_id: "Patient/example-01", // Mock patient ID
        namaste_code: selectedNamaste.NAMASTE_Code,
        namaste_display: selectedNamaste.Traditional_Term,
        icd_code: icdResults.biomed.code, // Using biomedical equivalent for main ICD code
        icd_display: icdResults.biomed.display
      };

      try {
        // NOTE: Your backend has this endpoint in the condition_router, not the root.
        // I will assume it should be /api/generate-fhir-condition from your router code
        const response = await api.post('/api/generate-fhir-condition', payload);

        // Let's add the extra metadata for a richer display as in the screenshot
        ...
        <div style={styles.resultsContainer}>
            ...
            <div style={{...styles.card, ...styles.resultCard}}>
                ...
                <div style={styles.referenceChip}>
                    {/* Mocking this part as it's not in the CSV */}
